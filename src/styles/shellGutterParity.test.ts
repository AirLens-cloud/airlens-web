...
describe('shell gutter parity — every page shell uses var(--pad-x) horizontally', () => {
  ...
  it('content.css shared shell (dispatch/article/blog) declares horizontal padding as var(--pad-x)', () => {
    // Arrange
    const css = readStyle('content.css')
    // Act
    const padding = paddingIn(
      css,
  ...
  it('insights.css mobile override keeps horizontal padding on var(--pad-x)', () => {
    // Arrange — the @media (max-width: 768px) override redeclares vertical
    // rhythm at a smaller step but must not diverge from the fluid gutter.
    ...
    // Act
    const padding = paddingIn(mobileBlock[1], String.raw`\.ins-shell`)
    // Assert
    for (const value of horizontalOf(padding)) {
      expect(value).toBe('var(--pad-x)')
    }
